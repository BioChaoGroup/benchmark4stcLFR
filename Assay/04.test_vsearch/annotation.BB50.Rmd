...
ggplot(selectAnno%>%filter(anno%in%multiAnno)) + 
  facet_wrap(~anno,ncol=2,scale="free") + theme_bw() +
  geom_segment(aes(x=sstart,xend=send,y=ID1,yend=ID1),size=2,alpha=.5) +
  geom_segment(aes(x=offset,xend=offset+QLEN,y=factor(node),yend=factor(node)),size=2,alpha=.5) +
  geom_segment(aes(x=offset+qstart,xend=sstart,y=factor(node),yend=ID1),linetype=2) +
  geom_segment(aes(x=offset+qend,xend=send,y=factor(node),yend=ID1),linetype=2) + 
  geom_text(aes(x=QLEN/2+offset,y=factor(node),label=pident),hjust=0.5)
