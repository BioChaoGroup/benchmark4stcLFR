...
grep ">" $SAM/VSEARCH/barrnap.LFRs.fasta|sed 's/>//;s/::/\t/'|sort -k2,2 |perl -e 'while(<>){@s=split; @b=split("_",$s[2]);
if($bi ne $b[0]){$r=1}elsif($gi ne $b[7]){$r++};$bi=$b[0];$ri=$b[2];
...
tag2 <- "RCA197M01_8"
rRNA2zymo.stat <- read.table(paste0(pfx,"/rRNA2zymo.stat"), col.names = c("BI","f","rRNA","unit","rLen","mLen","Ref"))
BI.lst <- read.table(paste0(pfx,"/Assemble_BI/ID.lst"), col.names = c("ID","BB","reads"))
BI2zymo.stat <- read.table(paste0(pfx,"/VSEARCH/contig.LFRs.bwa.sbb3"), col.names = c("count","rpb","LFRs","Refs"))
BI2LFR.stat <- read.table(paste0("../../Results/RCAs/RCA197FIN/",tag2,"/VSEARCH/contig.B01_2.bwa.sbb3"), col.names = c("count","rpb","LFRs","Refs"))

```

Curate
```{r}
BI.lst$BI <- sprintf("BI%08d",BI.lst$ID)

mdat <- merge(BI.lst,rRNA2zymo.stat,by="BI")

levels(mdat$Ref)[grep("Cryptococcus_neoformans",levels(mdat$Ref))] <- "Cryptococcus_neoformans_contigs"
levels(mdat$Ref)[grep("Saccharomyces_cerevisiae",levels(mdat$Ref))] <- "Saccharomyces_cerevisiae_contigs"

mdatS0 <- ddply(mdat,c("BI","reads"),summarise,fNum=max(f),rNum=max(rRNA),
                rLen=min(rLen),mLen=min(mLen),mCov=min(mLen/rLen),
                uType=sum(as.numeric(unique(unit))),Refs=length(unique(Ref)))

mdatS1 <- ddply(mdat%>%filter(mLen/rLen>0.5),c("BI","reads"),summarise,fNum=max(f),rNum=max(rRNA),
                rLen=min(rLen),mLen=min(mLen),mCov=min(mLen/rLen),
                uType=sum(as.numeric(unique(unit))),Refs=length(unique(Ref)))
...
```{r}
mdatS0$mCov1 <- round(mdatS0$mCov,1)
ggplot(mdatS0,aes(x=rLen,y=Refs,size=factor(mCov1),color=factor(rNum))) + geom_point(alpha=.5) +
  facet_grid(factor(uType)~.)

mdatS0$rLen1 <- round(mdatS0$rLen/200-0.5,0)*200
ggplot(mdatS0,aes(x=factor(rLen1),fill=factor(Refs))) + geom_bar(stat="count",position="stack") +
  facet_grid(.~factor(uType),scale="free",space="free") + theme(axis.text.x = element_text(angle=-45,vjust=0))

mdatS1 <- ddply(mdatS0,c("rLen1","uType","Refs"),summarise,count=length(BI))
data_m <- mdatS1 %>% group_by(rLen1,uType) %>% mutate(COUNT=sum(count)) %>% mutate(pct=round(100*count/COUNT,1))
ggplot(data_m,aes(x=factor(rLen1),y=pct,fill=factor(Refs))) + geom_bar(stat="identity",position="stack") +
...
ggplot(mdatS0,aes(x=factor(rLen1),fill=factor(Refs))) + geom_bar(stat="count",position="stack") +
  facet_grid(rNum~factor(uType),scale="free",space="free") + scale_y_log10() +
  theme(axis.text.x = element_text(angle=-45,vjust=0))
