...
Here I test the communities clustered by rpb(reads per bead) between 2 to 20 and mash distance from 0 to 0.02 (boundary not included):
```{r}
...
This figure pointed that when #beads per cluster were too small, it's almost impossible to get a good enough scaffolds with unique annotation. 3 or 4 cluster are seeem hybridized and all of them are mixed by *Fv* and *Le* (see below table). So my previous guess are totally worong :/  
