...
awk 'FNR>1{{print $5}}' $SAM/OTU/pred.RSUs.bwa.sbb1_$j|sort|uniq -c > $SAM/OTU/pred.RSUs.bwa.prop_$j
done;done
...
#test gff
test.Z1.barrnap.gff <- read.table("../../Results/JAN20/SAM/Z1/summary.BI.megahit.rRNA.fasta.barrnap.gff3",
                       col.names = c("CID","version","gene","ps","pe","evalue","strand","dot","name"),sep = "\t")
test.Z1.barrnap.gff$name <- as.character(test.Z1.barrnap.gff$name)
test.Z1.barrnap.gff$CID <- as.character(test.Z1.barrnap.gff$CID)

#test cluster
Z2.clip.clust.uc <- read.table("../../Results/JAN20/SAM/Z2/CLIP/all.clust.uc",as.is=T,
                               col.names = c("type","num","length","ident","strand","c6","c7","cigar","query","subject"))

#test quantification
Z2.anno <- read.table("../../Results/JAN20/SAM/Z2/CLIP/all.LOTU.map.unified.anno",sep="\t",
                      col.names = c("LOTU","rTax","idents","lens","mis","gap","tax","taxid","rank","path"))
Z2.prop <- read.table("../../Results/JAN20/SAM/Z2/CLIP/all.LOTU.bwa.prop",col.names = c("count","LOTU"))
Z2.faID <- read.table("../../Results/JAN20/SAM/Z2/CLIP/all.clust.logID",
                      col.names = c("LOTU","size","C75","CLEN","XLEN","MLEN"))
Z2.closeAnno <- read.table("../../Results/JAN20/SAM/Z2/CLIP/all.clust.blastn2DD6305.rRNA.m6.tsv",sep=" ",
                      col.names = c("LOTU","","rTax","idents","lens","mis","gap","qs","qe","ss","se","evalue","bs","qlen"))
Z8.prop <- read.table("../../Results/JAN20/SAM/Z8/CLIP/all.LOTU.bwa.prop_00",col.names = c("count","LOTU"))
...
ZS.filter.df.ref2.sum <- ddply(ZS.filter.df.ref2,"selectTax",summarise,delta.mean=mean(delta),pct.se=sd(delta))
ggplot(ZS.filter.df.ref2.sum,aes(x=selectTax,y=delta.mean,fill=selectTax)) +geom_bar(stat="identity") + 
