...
#quantification
metabbq smk -j -npk SAM/M{4,5,6}/KRAKEN/reads2mock.sp.prf
for i in M{4,5,6};do awk -v n=$i '{print n"\t"$0}' SAM/$i/KRAKEN/reads2mock.sp.prf;done > STAT/KRAKEN/MFR.reads2mock.sp.prf
metabbq smk -j -npk SAM/M{4,5,6}/KRAKEN/reads2db.sp.prf
for i in M{4,5,6};do awk -v n=$i '{print n"\t"$0}' SAM/$i/KRAKEN/reads2db.sp.prf;done > STAT/KRAKEN/MFR.reads2db.sp.prf
...
# MFR.clip.metadata.df <- read.table(
#   md5("../../Results/JAN20/STAT/CLIP/MFR.clip.metadata.tsv","94c212718996d4a240dae03dcc93374d"), 
#   sep="\t",col.names = c("SID","BID","K","cNo","flag","multi","oLen","cType","Ads","clen","RCAinfo")
# )
MFR.bead.metadata.df <- read.table(
  md5("../../Results/JAN20/STAT/CLIP/MFR.bead.metadata.tsv","01f5b42d7dbca3059415d8cff3843239"), 
  sep="\t",col.names = c("SID","BID","BLen","contigs","clips")
)
# MFR.LOTU.info <- read.table(
#   md5("../../Results/JAN20/STAT/CLIP/MFR.LOTU.log","2c3f0cc2f3fa032c7af1092789a1d0a1"),sep=" ",
#   col.names = c("SID","LOTU","size","S75","CL","MAXLEN","MEANLEN","SBP","P1D","D9P","COR","START","END","PMR")
# )

# MFR.mock.clip.anno <- read.table(md5("../../Results/JAN20/STAT/ANNO/MFR.clip2MockRef.clip.anno","16f7e465e2cec4e1acbeda33ac9fff62"),
#     sep="\t", col.names =c("SID","CLIP","tax","idents","lens","mis","gap","qs","qe","ss","se","eval","bit","qlen"))

MFR.mock.bead.anno <- read.table(md5("../../Results/JAN20/STAT/ANNO/MFR.clip2MockRef.bead.anno","3a5fda1be44cdf80ebd21c92295fdbd2"),
...
MFR.silva.bead.anno <- read.table(md5("../../Results/JAN20/STAT/ANNO/MFR.CLIP.map.merge.bead.anno","63b3890f3f9747d73a50c3fdb37e9a69"),comment.char = "",quote = "",sep="\t", col.names = c("SID","BINFO","tax","ident","mapLen","mismatch","gap","qs","qe","ss","se","eval","bitscore","qlen","slen","strands","qDesc","sDesc","covs","TopAnno","TANum","TALv","score1st","score2nd","ident1st","ident2nd","TANames","hybCt","hybLen","hybPct")
)
...
ggplot(MFR.bead.df,aes(x=BLen)) +geom_histogram(aes(fill=unit),position="stack",binwidth=100) +
...
MFR.reads2MFR.sp.prf <- read.table(
  md5("../../Results/JAN20/STAT/KRAKEN/MFR.reads2MFR.sp.prf","32ba81912f4b8e5f346e1ba0020fba45"),
