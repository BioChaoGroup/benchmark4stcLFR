...
#quantification
metabbq smk -j -npk SAM/Z{1,2,3}/KRAKEN/reads2mock.sp.prf
for i in Z{1,2,3};do awk -v n=$i '{print n"\t"$0}' SAM/$i/KRAKEN/reads2mock.sp.prf;done > STAT/KRAKEN/ZBR.reads2mock.sp.prf
metabbq smk -j -npk SAM/Z{1,2,3}/KRAKEN/reads2db.sp.prf
for i in Z{1,2,3};do awk -v n=$i '{print n"\t"$0}' SAM/$i/KRAKEN/reads2db.sp.prf;done > STAT/KRAKEN/ZBR.reads2db.sp.prf
...
ZBR.bead.metadata.df <- read.table(
  md5("../../Results/JAN20/STAT/CLIP/ZBR.bead.metadata.tsv","73143f768d66d3fe4998378459db5993"),
...
# )
# 
ZBR.mock.bead.anno <- read.table(md5("../../Results/JAN20/STAT/ANNO/ZBR.clip2MockRef.bead.anno","d9561b721f6e932adda5233b25641f5f"),
...
# ZBR.mock.LOTU.anno <- read.table(md5("../../Results/JAN20/STAT/ANNO/ZBR.LOTU.to.MockRef.bead.anno",
...
ZBR.silva.bead.anno <- read.table(
  md5("../../Results/JAN20/STAT/ANNO/ZBR.CLIP.map.merge.bead.anno","fc71e1f918294d8942ed4bacd2de3b2a"),comment.char = "",sep="\t", quote = "", fill = NA,col.names = c("SID","BINFO","tax","ident","mapLen","mismatch","gap","qs","qe","ss","se","eval","bitscore","qlen","slen","strands","qDesc","sDesc","covs","TopAnno","TANum","TALv","score1st","score2nd","ident1st","ident2nd","TANames","hybCt","hybLen","hybPct")
)

# ZBR.silva.LOTU.anno <- read.table(md5("../../Results/JAN20/STAT/ANNO/ZBR.LOTU.map.merge.anno",
...
ZBR.KK.neg <- read.table(md5("../../Results/JAN20/STAT/KRAKEN/rDNA.bead.neg.stat",
  "c221b807e927ff326b39575ab308462e"), col.names = c("SID","count","rpb","negCount"))

ZBR.id95def4.BID <- read.table(md5("../../Results/JAN20/STAT/CLIP/id95def4.clust.H.BID",
...
ggplot(ZBR.bead.df,aes(x=BLen)) +geom_histogram(aes(fill=unit),position="stack",binwidth=100) +
...
```{r}
ge1st <- sapply(strsplit(as.character(ZBR.bead.df$tax),"_"),"[",1)
ge2nd <- sapply(strsplit(as.character(ZBR.bead.df$TANames),"_"),"[",1)
ZBR.bead.df$hybEst <- ifelse(ZBR.bead.df$ident2nd>ZBR.bead.df$ident1st&!is.na(ge2nd)&ge2nd!=ge1st,T,F)
...
ZBR.bead.df$S75.2 <- 2^round(log2(ZBR.bead.df$S75),0)

ggplot(ZBR.bead.df,aes(x=S75,fill=SID)) +geom_histogram(position="dodge") +
  scale_x_log10() + annotation_logticks(side="b")
ggplot(ZBR.bead.df,aes(y=ident,x=factor(S75.2),fill=SID)) +geom_boxplot(aes(fill=SID)) +
  scale_y_continuous(breaks=seq(94,102,1),limits=c(94,100))
ggplot(ZBR.bead.df%>%filter(S75<20),aes(y=ident,x=factor(S75),fill=SID)) +geom_boxplot(aes(fill=SID)) +
  scale_y_continuous(breaks=seq(94,102,1),limits=c(94,100))

ZBR.bead.df$SBP.2 <- 2^round(log2(ZBR.bead.df$SBP),0)
ggplot(ZBR.bead.df,aes(y=ident,x=factor(SBP.2),fill=SID)) +geom_boxplot(aes(fill=SID)) +
  scale_y_continuous(breaks=seq(94,102,1),limits=c(94,100))
ggplot(ZBR.bead.df%>%filter(SBP<32),aes(y=ident,x=factor(SBP))) +geom_boxplot() +
  scale_y_continuous(breaks=seq(94,102,1),limits=c(94,100))

ZBR.bead.df$P1D.2 <- 2^round(log2(ZBR.bead.df$P1D),0)
ggplot(ZBR.bead.df,aes(y=ident,x=factor(P1D.2),fill=SID)) +geom_boxplot(aes(fill=SID)) +
  scale_y_continuous(breaks=seq(94,102,1),limits=c(94,100))
ggplot(ZBR.bead.df%>%filter(P1D<32),aes(y=ident,x=factor(P1D))) +geom_boxplot() +
  scale_y_continuous(breaks=seq(94,102,1),limits=c(94,100))

ZBR.bead.df$D9P.2 <- 2^round(log2(ZBR.bead.df$D9P),0)
ggplot(ZBR.bead.df,aes(y=ident,x=factor(D9P.2),fill=SID)) +geom_boxplot(aes(fill=SID)) +
  scale_y_continuous(breaks=seq(94,102,1),limits=c(94,100))

ggplot(ZBR.bead.df,aes(x=CL,fill=unit)) +geom_histogram(position="stack",binwidth=50)  + facet_grid(.~SID)
ggplot(ZBR.bead.df,aes(x=COR,fill=unit)) +geom_histogram(position="stack",binwidth=50)  + facet_grid(.~SID)
ggplot(ZBR.bead.df%>%filter(CL>499),aes(x=CL,fill=unit)) +geom_histogram(position="stack",binwidth=100) +
  theme(legend.position = c(0.99,.9),legend.justification = c(1,1))

```

```{r}
ZBR.bead.df$KMERG <- ifelse(ZBR.bead.df$KMER<=111,"<=111",ZBR.bead.df$KMER)

ggplot(ZBR.bead.df%>%filter(!is.na(flag)),aes(x=multi,y=ident)) + 
  stat_density_2d(aes(fill = stat(nlevel)), geom = "polygon") + 
  scale_fill_gradientn(colours=rev(brewer.pal(11,"Spectral"))) +
...
```{r}
ZBR.bead.df$ident5 <- round(ZBR.bead.df$ident+0.49,)-0.5
ZBR.bead.df$ident5 <- ifelse(ZBR.bead.df$ident5<95,94.5,ZBR.bead.df$ident5)

ggplot(ZBR.bead.df%>%filter(COR>999&flag!=0&D9P/COR<0.01),aes(x=ident5)) +
...
badf.stat2 <- ddply(
  ddply(ZBR.bead.df%>%filter(COR>999&flag!=0&D9P/COR<0.01),c("SID","ident5"),summarise,count=length(SID)),
  c("ident5"),summarise,mean.count=mean(count),sd=sd(count))

badf.stat3 <- ddply(
  ddply(ZBR.bead.df%>%filter(COR>999&flag!=0&D9P/COR<0.01),c("SID","ident5","unit"),summarise,count=length(SID)),
...
ZBR.bead.df$CORG <- round(ZBR.bead.df$COR/500-0.49,0)*500

badf.stat4 <- ddply(
  ddply(ZBR.bead.df%>%filter(COR>999&flag!=0&D9P/COR<0.01),c("SID","CORG","ident5","unit"),summarise,count=length(SID)),
...
badf.stat5 <- ddply(
  ddply(ZBR.bead.df%>%filter(COR>999&flag!=0&D9P/COR<0.01),c("SID","flag","ident5","unit"),summarise,count=length(SID)),
...
ZBR.compare.win <- ddply(ZBR.compare.anno,c("SID","BLenK"),summarise,hybT=length(which(hybG==T)),TAM=length(which(TopAnno.y=="multi")),count=length(BID))
...
ZBR.reads2ZBR.sp.prf <- read.table(
  md5("../../Results/JAN20/STAT/KRAKEN/ZBR.reads2ZBR.sp.prf","72291069e291d5edf55f56b565110f81"),
