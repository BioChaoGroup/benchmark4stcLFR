...
# merge ref anno
cat SAM/MFR/ANNO/{mock,CLIP}.map.merge.bead.anno > SAM/MFR/ANNO/refbead.map.merge.bead.anno
...
metabbq IO clade2tree -i SAM/MFR/CLIP/refbead.clust.fa -a SAM/MFR/ANNO/refbead.map.merge.bead.anno \
...
MFR.r2BEAD.species.read.bracken <- read.table(
  md5("../../../Results/JAN20/STAT/KRAKEN/MFR.reads2BEAD.read.bracken","3d431a37cebb0d3f822918ec01a1a8b6"),
...
MFR.r2BEAD.species.bead.bracken <- read.table(
  md5("../../../Results/JAN20/STAT/KRAKEN/MFR.reads2BEAD.bead.bracken","22680fc3471042f32a690eacea68a18c"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

MFR.r2refbead.species.read.bracken <- read.table(
  md5("../../../Results/JAN20/STAT/KRAKEN/MFR.reads2refbead.read.bracken","14770e6841cd589c8e6229d0d63e52c9"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

MFR.r2refbead.species.bead.bracken <- read.table(
  md5("../../../Results/JAN20/STAT/KRAKEN/MFR.reads2refbead.bead.bracken","017b00eb5169a12c6adfdd9f1d748b1e"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))
...
topTax <- rev(levels(reorder(as.character(droplevels(MFR.r2BEAD.b.sp.anno$spName)),MFR.r2BEAD.b.sp.anno$pct,sum)))[1:10]
...
MFR.r2BEAD.b.sp.ref.anno$spName <- as.factor(MFR.r2BEAD.b.sp.ref.anno$topTax)
...
MFR.r2BEAD.b.sp.ref.anno.stat <- ddply(MFR.r2BEAD.b.sp.ref.anno,c("CREF","LIB","tech","spName"),summarise,
                                       count=length(delta),pct=mean(pct),TC=mean(TC),
                                       sd=sd(delta),mean.delta=mean(delta),
                                       sd.k=sd(delta.k),mean.delta.k=mean(delta.k))
MFR.r2BEAD.b.sp.ref.anno.stat$spName <- reorder(MFR.r2BEAD.b.sp.ref.anno.stat$spName,MFR.r2BEAD.b.sp.ref.anno.stat$mean.delta,min)

MFR.r2BEAD.b.sp.ref.anno.stat$spByPct <- reorder(MFR.r2BEAD.b.sp.ref.anno.stat$spName,MFR.r2BEAD.b.sp.ref.anno.stat$pct,min)
ggplot(MFR.r2BEAD.b.sp.ref.anno.stat,
       aes(x=spByPct,y=pct,fill=LIB)) +geom_bar(stat="identity",position="dodge") +
...
ggplot(MFR.r2BEAD.b.sp.ref.anno.stat,
       aes(x=LIB,y=mean.delta.k,fill=LIB)) +geom_bar(stat="identity",position="dodge") + 
  geom_errorbar(aes(ymin=mean.delta.k-sd.k,ymax=mean.delta.k+sd.k)) +
  coord_flip() + facet_grid(spName~CREF+tech)
```


```{r}
MFR.r2BEAD.b.sp.ref.anno$delta.k <- MFR.r2BEAD.b.sp.ref.anno$pct.k - MFR.r2BEAD.b.sp.ref.anno$TC16S

MFR.r2BEAD.b.sp.ref.anno.stat.k <- ddply(MFR.r2BEAD.b.sp.ref.anno,c("CREF","UID","SID","LIB","tech","spName"),summarise,mean.delta=mean(delta.k),sd=sd(delta.k))
MFR.r2BEAD.b.sp.ref.anno.stat.k$spName <- reorder(MFR.r2BEAD.b.sp.ref.anno.stat.k$spName,MFR.r2BEAD.b.sp.ref.anno.stat.k$mean.delta,min)

ggplot(MFR.r2BEAD.b.sp.ref.anno.stat.k%>%filter(spName%in%zymo.bac),
       aes(x=spName,y=mean.delta,fill=LIB)) +geom_bar(stat="identity",position="dodge") + coord_flip() + facet_grid(.~CREF+tech)
