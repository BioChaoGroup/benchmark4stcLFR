...
# merge ref anno
cat SAM/MBR/ANNO/{D6305,CLIP}.map.merge.bead.anno > SAM/MBR/ANNO/refbead.map.merge.bead.anno
...
metabbq IO clade2tree -i SAM/MBR/CLIP/refbead.clust.fa -a SAM/MBR/ANNO/refbead.map.merge.bead.anno \
...
MBR.r2BEAD.species.read.bracken <- read.table(
  md5("../../../Results/JAN20/STAT/KRAKEN/MBR.reads2BEAD.read.bracken","e69b506df2246930de5c6091fc70382a"),
...
MBR.r2BEAD.species.bead.bracken <- read.table(
  md5("../../../Results/JAN20/STAT/KRAKEN/MBR.reads2BEAD.bead.bracken","274078328b518711d9f5b6edd37f9a99"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

MBR.r2refbead.species.read.bracken <- read.table(
  md5("../../../Results/JAN20/STAT/KRAKEN/MBR.reads2refbead.read.bracken","7f3e5bef63806516ce289bc62ab7633b"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

MBR.r2refbead.species.bead.bracken <- read.table(
  md5("../../../Results/JAN20/STAT/KRAKEN/MBR.reads2refbead.bead.bracken","1d35339e24e57be2ceb2fbdaa7df4ebf"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))
...
```{r}
MBR.r2BEAD.b.sp.ref.anno <- merge(MBR.r2BEAD.b.sp.anno,zymo.copy.num,by.x="spName",by.y="species")
...
MBR.r2BEAD.b.sp.ref.anno.stat.k <- ddply(MBR.r2BEAD.b.sp.ref.anno,c("CREF","UID","SID","LIB","tech","spName"),summarise,mean.delta=mean(delta.k),sd=sd(delta.k))
