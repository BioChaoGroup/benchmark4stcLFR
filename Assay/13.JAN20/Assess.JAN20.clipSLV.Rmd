...
if(file.exists(RDFile1)||file.exists(RDFile1)){
  load(md5(RDFile1,"4035d72341ae92c5abc5c46c8c6e0d46"))
  load(md5(RDFile2,"035ca071b8f6b0bcd4beb1d7be26983e"))
}else{
  ...
  clip.slv.stat.bead <-read.table("../../Results/JAN20/STAT/megahit.clip.slv.stat.beads",
    sep="\t",as.is = T,quote = "",comment.char="",
    col.names = c("SID","BB", "clips","annos", "clv", "cName","path"))
...
clv.path <- as.data.frame(t(sapply(clip.slv.sc$cTax,pFun,USE.NAMES=F)))
colnames(clv.path) <- the.names
clv.path.df <- cbind(csdf[,1:6],clv.path)
```

Sum clips to beads:
```{r}
clip.slv.sb <- merge(metadata,clip.slv.stat.bead,by="SID")
cbdf <- merge(bb_clips,clip.slv.sb,by=c("SID","BB"))
cbdf$SID <- droplevels(cbdf$SID)
