...
metabbq smk -j --configfile config.F.yaml -pk SAM/{S,O}{4,5,6}/CLIP/all.ITS.positions.sum.tsv
...
metabbq smk --configfile config.F.yaml -j -npk SAM/{S,O}{4,5,6}/ANNO/LOTU.map.merge.anno
for i in {S,O}{4,5,6};do sed 's/\S\+=//g' SAM/$i/ANNO/LOTU.map.merge.anno|awk -v n=$i '{print n"\t"$0}' ;done > STAT/ANNO/SFR.LOTU.map.merge.anno


#SNV
metabbq smk --force -j -pk SAM/{S,O}{4,5,6}/CLIP/clips2REF.bcf.tsv
...
done;done > STAT/PROFILE/SFR.LOTU.bwa.sbb1_rand3

for i in {S,O}{4,5,6};do metabbq diversityEst -e 0 -s $i -t 100 -d SAM/$i -m SAM/SFR -o SAM/$i/CLIP/divEst.tsv & done
for i in {S,O}{4,5,6};do awk -v n=$i '{print n"\t"$0}' SAM/$i/CLIP/divEst.tsv;done > STAT/CLIP/SFR.divEst.tsv
...
SFR.bead.metadata.df <- read.table(
  md5("../../Results/JAN20/STAT/CLIP/SFR.bead.metadata.tsv","a9a3fe759d99eed99ff2b1f650a57b6f"),
  sep="\t",col.names = c("SID","BID","BLen","contigs","clips")
)

SFR.barrnap.gff3.tsv <- read.table(
  md5("../../Results/JAN20/STAT/CLIP/SFR.all.barrnap.sum.tsv","a70ff3081d2807ed39a4e65cee3b9da0"),
...
SFR.itsx.pos.tsv <- read.table(
  md5("../../Results/JAN20/STAT/CLIP/SFR.all.ITS.positions.sum.tsv","baa34b6e503c33694a66d7af75c7c30e"),
  col.names = c("SID","BID","SSU","ITS1","S5.8","ITS2","LSU","scoreA","scoreB")
)

SFR.silva.bead.anno <- read.table(
  md5("../../Results/JAN20/STAT/ANNO/SFR.CLIP.map.merge.bead.anno","453324bd2239d0794ef2fce82fe422e3"),comment.char = "",sep="\t", quote = "", 
...
# SFR.silva.LOTU.anno <- read.table(md5("../../Results/JAN20/STAT/ANNO/SFR.LOTU.map.merge.anno",
...
# )
# 
SFR.clip2REF.bcf <- read.table(md5("../../Results/JAN20/STAT/CLIP/SFR.clips2REF.bcf.tsv",
...
# 
# SFR.prop <- read.table(md5("../../Results/JAN20/STAT/PROFILE/SFR.LOTU.bwa.sbb1_rand3","12564411f41155ab37b0a07f92d0c57c"),sep="\t",
...
```{r}
SFR.F.subunit.df <- SFR.barrnap.gff3.tsv[,c(1,2,5:8,11,12,13)]
...
SFR.slv.bead.all.df$unit <- factor(
  ifelse(SFR.slv.bead.all.df$LSU>5,1,0) + ifelse(SFR.slv.bead.all.df$ITS>25,2,0) + ifelse(SFR.slv.bead.all.df$SSU>5,4,0),
  levels=seq(0,7)
)

levels(SFR.slv.bead.all.df$unit) <- c("partial(<50%)","LSU","ITS","LSU+ITS","SSU","LSU+SSU","SSU+ITS","ALL")
...
```{r}
SFR.slv.bead.all.df$identG <- ifelse(SFR.slv.bead.all.df$ident == 100,100,ifelse(SFR.slv.bead.all.df$ident >=99.5,99.5,
       ifelse(SFR.slv.bead.all.df$ident <97,96,round(SFR.slv.bead.all.df$ident-0.5,0))))
...
pct.identG$identG <- factor(pct.identG$identG,levels=c(96,97,98,99,99.5,100))
...
```{r}
combine.unit.df <- merge(merge(SFR.itsx.df,SFR.F.subunit.df,by=c("SID","BID"),all.x=T),SFR.slv.bead.df[,c(1,2,27:29)],by=c("SID","BID"),all.x=T)
...
ggplot(tmp.bead.df,aes(x=qlen,color=TopAnno)) + geom_density(aes(y=..density..*clips)) + xlim(c(0,4000))
...
** load**
```{r}
ranks.map <- read.table("../../Results/JAN20/SAM/SFR/KRAKEN/db/data/ranks.tsv",comment.char = "",
...
SFR.r2SFR.species.bracken <- read.table(
  md5("../../Results/JAN20/STAT/KRAKEN/SFR.reads2SFR.bracken","5268517b9cc700e739bd85a35c5350aa"),
