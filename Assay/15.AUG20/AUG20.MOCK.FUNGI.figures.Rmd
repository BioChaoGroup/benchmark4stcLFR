...
MFR.mock.bead.anno <- read.table(
  md5("../../Results/AUG20/STAT/ANNO/MFR.clip2MockRef.bead.anno","5f59962bdd3a579077d1efd547139171"),
...
MFR.silva.bead.anno <- read.table(
  md5("../../Results/AUG20/STAT/ANNO/MFR.CLIP.map.merge.bead.anno","bdb11ffb94ee81a69cb755d3fc16fd3f"),
  comment.char = "",quote = "",sep="\t", col.names = c("SID","BINFO","tax","ident","mapLen","mismatch","gap","qs","qe","ss","se","eval","bitscore","qlen","slen","strands","qDesc","sDesc","covs","taxonID","TopAnno","TANum","TALv","score1st","score2nd","ident1st","ident2nd","TANames","hybCt","hybLen","hybPct")
)
...
```
```{r}
MFR.compare.anno$hybG <- ifelse(MFR.compare.anno$hybCt>0&MFR.compare.anno$hybLen/MFR.compare.anno$BLen>0.1,T,F)
ggplot(MFR.compare.anno%>%filter(hybCt>0),aes(x=hybLen,fill=hybG)) + geom_histogram()

MFR.compare.anno$BLenK <- round(MFR.compare.anno$BLen/100-0.5,0) * 100
MFR.compare.anno.cut <- MFR.compare.anno
...
ggplot(MFR.compare.win,aes(x=BLenK,linetype=SID)) + 
  ...
  xlim(c(100,4000)) + scale_y_continuous(breaks=seq(0,1,.2))
...
ggplot(MFR.compare.anno%>%filter(hybCt>0),aes(x=BLen,y=hybLen)) + 
  stat_density_2d(aes(fill = stat(nlevel)), geom = "polygon") + 
  scale_fill_gradientn(colours=rev(brewer.pal(11,"Spectral"))) +
...
MFR.compare.unit <- MFR.compare.anno.cut2 %>% 
  group_by(SID,BLenK,unit.x) %>% summarise( 
    hybT=length(which(hybG==T)), TAM=length(which(TopAnno.y=="multi")),
    count=length(BID), pct.hyb=hybT/count, pct.multi=TAM/count,
    pct.normal=length(which(hybG==F&TopAnno.y!="multi"))/count
  ) %>% melt(
    id.vars = c("SID","BLenK","unit.x"),measure.vars = c("pct.hyb","pct.multi","pct.normal"),
    variable.name = "type", value.name = "ratio"
  )

ggplot(MFR.compare.unit,aes(x=BLenK,linetype=SID)) + 
  geom_line(aes(y=ratio,color=type)) + facet_grid(unit.x~.) +
  xlim(c(100,3000)) + scale_y_continuous(breaks=seq(0,1,.2))
