...
metabbq smk -j -npk SAM/Z{1,2,3}/ANNO/clip2MockRef.clip.anno
...
ZBR.mock.bead.anno <- read.table(md5("../../Results/AUG20/STAT/ANNO/ZBR.clip2MockRef.bead.anno","6b3b5532478ac719dfa475ab1442cc23"),
...
zymo.rRNA.barrnap.bed <- read.table("../../Results/AUG20/etc/D6305.rRNA.barrnap.bed",sep="\t",
                                    col.names = c("CHR","start","end","unit","shift","plus","START","END","color"))
...
p.mbr.rca.units.hist <- ggplot(ZBR.bead.df,aes(x=BLen)) +geom_histogram(aes(fill=unit),position="stack",binwidth=100) +
  ...
  xlab("bead-barcoded-read assembly length")
...
```{r}
summary(ZBR.bead.df[,c("BID","BLen","contigs","clips")])
...
```{r}
plot_grid(p.mbr.rca.units.hist,plot_grid(p.mbr.rca.qual.pie,p.mbr.rca.units.pie,labels=c("b","c")),
          ncol=1,labels=c("a",""))
```

```{r}
if(do.write){
  if(!dir.exists("../../Results/AUG20/FIG")){dir.create("../../Results/AUG20/FIG")}
...
# Abundance bias (assessed by Beads)
```{r}
ZBR.mock.bead.stat <- ddply(ZBR.mock.bead.anno,c("SID","tax"),summarise,count=length(BID))
ZBR.mock.bead.stat$tax <- sub("_"," ",ZBR.mock.bead.stat$tax)
...
```
# close ref TA and hyb accessment
```{r}
p.ref.TA.hist <- ggplot(ZBR.bead.df,aes(x=BLen,fill=TopAnno)) + geom_histogram(position="dodge") + 
  xlim(c(0,4000)) + ggtitle("Annotaion consistency to close reference")

p.ref.hyb.hist <- ggplot(ZBR.bead.df,aes(x=BLen,fill=factor(hybCt==0))) + geom_histogram()
plot_grid(p.ref.TA.hist,p.ref.hyb.hist,labels=c("a","b"),ncol=1)
ZBR.ref.df <- ZBR.bead.df
ZBR.ref.df$BLenK <- round(ZBR.ref.df$BLen/100-0.5,0) * 100
ZBR.ref.df$hybG <- ifelse(ZBR.ref.df$hybCt>0&ZBR.ref.df$hybLen/ZBR.ref.df$BLen>0.1,T,F)
ZBR.ref.win <- ddply(ZBR.ref.df,c("SID","BLenK"),summarise,hybT=length(which(hybG==T)),TAM=length(which(TopAnno=="multi")),count=length(BID),pct.hyb=hybT/count,pct.multi=TAM/count)

ZBR.ref.win <- ZBR.ref.df %>% 
  group_by(SID,BLenK) %>% summarise( 
    hybT=length(which(hybG==T)), TAM=length(which(TopAnno=="multi")),
    count=length(BID), pct.hyb=hybT/count, pct.multi=TAM/count,
    pct.normal=length(which(hybG==F&TopAnno!="multi"))/count
  ) %>% melt(
    id.vars = c("SID","BLenK"),measure.vars = c("pct.hyb","pct.multi","pct.normal"),
    variable.name = "type", value.name = "ratio"
  )

ZBR.ref.win.mean <- ZBR.ref.win %>% 
  group_by(BLenK,type) %>% 
  summarise( sd=sd(ratio),ratio=mean(ratio) )

ggplot(ZBR.ref.win.mean,aes(x=BLenK)) + 
  geom_line(aes(y=ratio,color=type)) + 
  geom_ribbon(aes(ymax=ratio+sd,ymin=ratio-sd,fill=type),alpha=.3) +
  xlim(c(200,4000)) + scale_y_continuous(breaks=seq(0,1,.2),limits=c(0,1))
```
```{r}
p.ambiguous <- ggplot(ZBR.ref.win.mean,aes(x=BLenK)) + 
  geom_line(aes(y=ratio,color=type)) + 
  geom_ribbon(aes(ymax=ratio+sd,ymin=ratio-sd,fill=type),alpha=.3) +
  xlim(c(200,4000)) + scale_y_continuous(breaks=seq(0,1,.2),limits=c(0,1))
...
p.ref.TA.hist <- ggplot(ZBR.bead.df,aes(x=BLen,fill=TopAnno)) + geom_histogram(position="dodge") + 
  xlim(c(0,4000)) + ggtitle("Annotaion consistency to close reference")
p.slv.TA.hist <- ggplot(ZBR.slv.bead.df,aes(x=qlen,fill=TopAnno)) + geom_histogram(position="dodge") + 
  xlim(c(0,4000)) + ggtitle("Annotaion consistency to open reference (SILVA 138)")
plot_grid(p.ref.TA.hist,p.slv.TA.hist,labels=c("a","b"),ncol=1)
...
```{r}
ZBR.compare.anno$hybG.close <- ifelse(ZBR.compare.anno$hybCt.x>0&ZBR.compare.anno$hybLen.x > 400,T,F)
ZBR.compare.anno$hybG.open  <- ifelse(ZBR.compare.anno$hybCt.y>0&ZBR.compare.anno$hybLen.y > 400,T,F)

p.hyb.c <- ggplot(ZBR.compare.anno,aes(x=BLen,fill=hybG.close)) + geom_histogram()
p.hyb.cf <- ggplot(ZBR.compare.anno,aes(x=BLen,fill=hybG.close)) + 
  geom_histogram(position="fill") + xlim(c(0,5000))
p.hyb.o <- ggplot(ZBR.compare.anno,aes(x=BLen,fill=hybG.open)) + geom_histogram()
p.hyb.of <- ggplot(ZBR.compare.anno,aes(x=BLen,fill=hybG.open)) + 
  geom_histogram(position="fill") + xlim(c(0,5000))
...
ZBR.compare.anno.cut <- ZBR.compare.anno%>%filter(BLenK>=100)
...
ZBR.compare.win$group <- ifelse(grepl(".c",ZBR.compare.win$type,fixed = T),"cloesRefs","openRefs")
levels(ZBR.compare.win$type) <- rep(c("hybridized","multi-best annotation","perfect"),2)
...
ZBR.compare.win.mean$group <- ifelse(ZBR.compare.win.mean$group=="cloesRefs","full-length rRNA genes","public partial SSU/LSU genes")
levels(ZBR.compare.win.mean$type) <- c("Chimera","Multiple best taxonomies","Perfect assigned")
p.ambiguous.bac <- ggplot(ZBR.compare.win.mean,aes(x=BLenK)) + 
...
if(do.write){
  ggsave(p.ambiguous.bac,width=8,heigh=3, file="../../Results/AUG20/FIG/p.ambiguous.bac.line.pdf")
}
```
```{r}
ZBR.compare.unit <- ZBR.compare.anno.cut %>% 
  group_by(SID,unit3,BLenK) %>% summarise( 
    hybT=length(which(hybG==T)), TAM=length(which(TopAnno.y=="multi")),
    count=length(BID), pct.hyb=hybT/count, pct.multi=TAM/count,
    pct.normal=length(which(hybG==F&TopAnno.y!="multi"))/count
  ) %>% melt(
    id.vars = c("SID","unit3","BLenK"),measure.vars = c("pct.hyb","pct.multi","pct.normal"),
    variable.name = "type", value.name = "ratio"
  )

ggplot(ZBR.compare.unit,aes(x=BLenK,linetype=SID)) + 
  geom_line(aes(y=ratio,color=type)) + facet_grid(unit3~.) +
  xlim(c(0,4000))
...
sFUN <- function(d){
  ind <- seq(d[1,2],d[1,3])
  res <- data.frame(CHR=d[1,1],POS=ind,unit=d[1,4])
  return(res)
}
ZBR.clip2REF.bcf.df <- merge(ZBR.clip2REF.bcf,ddply(zymo.rRNA.barrnap.bed,c("CHR","start"),sFUN),by=c("CHR","POS"))
droplevels(ZBR.clip2REF.bcf.df$unit)
p.zymo.depth.ribbon <- ggplot(ZBR.clip2REF.bcf.df%>%filter(unit!="5S_rRNA"),aes(x=POS)) +
  geom_ribbon(aes(x=POS,ymin=0,ymax=DP,linetype=SID,fill=unit),alpha=.5) +
  geom_line(aes(y=DP,linetype=SID),alpha=.5,size=.5) +
  facet_wrap(~CHR,ncol=2,scale="free_y") + theme_bw() + scale_fill_brewer(palette="Set1")
...
# EXPORT (RUN MANUALLY!)
```{r,eval=FALSE}
ggsave(p.zymo.depth.ribbon, height=6,width=8,file="../../Results/AUG20/FIG/supp.ZBR.zymo.coverage.ribbon.pdf")
```
