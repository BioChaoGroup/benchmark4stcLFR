...
done > ../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2SUS.bead.$j.bracken & done
...
for db in {SILVA138.1SSU,SILVA138.1LSU,UNITE8.2,SUS,BEAD,BEAD+};do
  for SAM in {S,O};do 
    echo sort -S 24G -T .tmp -R --random-source config.yaml SAM/{$SAM\1,$SAM\2,$SAM\3}/KRAKEN/reads2$db.kraken2 \| split -d -a 3 -l 5000000 - SAM/SBR/KRAKEN/rand/pool/$db.$SAM.
    echo sort -S 24G -T .tmp -R --random-source config.F.yaml SAM/{$SAM\4,$SAM\5,$SAM\6}/KRAKEN/reads2$db.kraken2 \| split -d -a 3 -l 5000000 - SAM/SFR/KRAKEN/rand/pool/$db.$SAM.
  done
done > bat.sort.sh
...
use strict;
my(%HS,%SUM);
open REF,"<../../Results/AUG20/STAT/KRAKEN/uniq.taxonSet.taxID.list.tsv";
...
## Load from RData
```{r,load}
if(file.exists("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.r2multi.load.RData")){
  load(md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.r2multi.load.RData",
           "4c4f25184a9d8f79089db42a7c8ff837"))
}else{
...
ref.topRank.map <- rbind(
  cbind(DOM="BAC",BEAD.B.topRank.map),
  cbind(DOM="EUK",BEAD.F.topRank.map)
)
...
# BEAD
SXR.r2BEAD.S.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2BEAD.bead.S.bracken","ebd3eceafdd6c4d7bf51393426ceb83f"),
...
SXR.r2BEAD.G.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2BEAD.bead.G.bracken","86437d3e08043a4260bb2c97d488f108"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

SXR.r2BEAD.P.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2BEAD.bead.P.bracken","dc4825bd9c1f55bd1ca769e99e52b2b2"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

# BEAD+
SXR.r2BEADP.S.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2BEAD+.bead.S.bracken","0646fa52358ecef29ba13732473210ab"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

SXR.r2BEADP.G.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2BEAD+.bead.G.bracken","9d2a098a994da1b3447b519e5e4105e2"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

SXR.r2BEADP.P.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2BEAD+.bead.P.bracken","0cca585f037a6ed54508829bacd412fa"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

# SUS (SILVA/UNITE)
SXR.r2SUS.S.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2SUS.bead.S.bracken","7b7c55ab277168080f6e012624d5d3c6"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

SXR.r2SUS.G.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2SUS.bead.G.bracken","744456aa2b9c01da7caea9f2060a5ea6"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))

SXR.r2SUS.P.bracken <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.reads2SUS.bead.P.bracken","87c1bb25a898a2d2b520a971495891ad"),
  as.is = T,quote = "",comment.char = "", sep="\t",
  col.names = c("SID","name","taxID","rank","kraken_assigned_reads","added_reads","new_est_reads","fraction_total_reads"))
...
SXR.reads2multi.bracken <- rbind(
  cbind(DB="BEAD",profRank="Species",SXR.r2BEAD.S.bracken),
  cbind(DB="BEAD",profRank="Genus",SXR.r2BEAD.G.bracken),
  cbind(DB="BEAD",profRank="Phylum",SXR.r2BEAD.P.bracken),
  ...
  cbind(DB="PUBS",profRank="Species",SXR.r2SUS.S.bracken),
  cbind(DB="PUBS",profRank="Genus",SXR.r2SUS.G.bracken),
  cbind(DB="PUBS",profRank="Phylum",SXR.r2SUS.P.bracken)   
)
...
save(SXR.r2m.bracken.more, file="../../Results/AUG20/STAT/KRAKEN/SXR.RCA.r2multi.load.RData")
tools::md5sum("../../Results/AUG20/STAT/KRAKEN/SXR.RCA.r2multi.load.RData")
```
...
count.stat0 <-SXR.r2m.bracken.more%>%group_by(UID,DOM,DB,SAM,profRank,taxonSet,REP)%>%
...
p.obs.db.compare.bar <- ggplot(count.stat0,aes(x=DB,y=observe)) + geom_bar(stat="identity",aes(fill=taxonSet)) +
...
if(do.write){
  ggsave(width=6,height=6,file="../../Results/AUG20/FIG/p.obs.db.compare.bar.pdf")
 }
```

```{r}
p.abun.db.compare.bar <- ggplot(count.stat0,aes(x=DB,y=abun)) + geom_bar(stat="identity",aes(fill=taxonSet)) +
  geom_text(aes(y=1-apos+abun/2,label=round(abun,2)))+
  scale_fill_brewer(palette="Set1") +
  facet_grid(profRank~DOM,scale="free_y") + theme_bw()

p.abun.db.compare.bar
if(do.write){
  ggsave(p.abun.db.compare.bar,width=6,height=6,file="../../Results/AUG20/FIG/p.abun.db.compare.bar.pdf")
 }
```
...
count.anno.df <-SXR.r2m.bracken.more %>% filter(DB=="JOIN"&profRank!="Species") %>%
  group_by(UID,DB,taxID)%>% transform(reObs=ifelse(new_est_reads>0,1,0))%>%
  group_by(UID,DOM,DB,profRank)%>% transform(sumObs=sum(reObs))%>% filter(sumObs>1)

count.anno.stat <- count.anno.df%>%
  group_by(UID,DOM,DB,SAM,profRank,domain,REP,taxonSet)%>%
  summarise(observe=length(unique(taxID)),beads=sum(new_est_reads)) %>%
...
p.obs.bar <- ggplot(count.anno.stat,aes(x=domain,y=observe)) + coord_flip() +
  ...
  facet_grid(DOM~profRank,scale="free_x") + ylab("Observed taxon nodes at each rank's level, on average") + scale_fill_brewer(palette = "Set1") +
  theme_bw()

p.obs.bar

if(do.write){
  ggsave(p.obs.bar,width=10,height=4,file="../../Results/AUG20/FIG/obs.sp.barplot.pdf")
}
```

```{r}
count.set.stat2 <-SXR.r2m.bracken.more%>%group_by(UID,DOM,DB,SAM,profRank,REP,taxonSet)%>%
  summarise(observe=length(taxID),beads=sum(new_est_reads))
ggplot(count.set.stat2,aes(x=DB,y=observe)) + 
  geom_boxplot(aes(fill=taxonSet)) + geom_jitter(aes(color=taxonSet,shape=SAM)) +
  facet_grid(profRank~DOM+taxonSet,scale="free_y") + scale_y_log10()
```
...
SXR.sp.rep.count.df <- dcast(SXR.r2m.bracken.more,
  DOM+DB+profRank+SAM+taxID+name+members+domain+phylum+findAnno+PBA_support~REP,
...
SXR.sp.rep.abun.dfd <- dcast(SXR.sp.rep.abun.dfm,
  DOM+DB+profRank+kk2Method+SAM+taxID+name+members+domain+phylum+genus+treeDist+burgDist+findAnno+PBA_support~REP,
...
ggplot(SXR.sp.rep.method.abun.dfs,aes(x=RepX,y=RepY,color=findAnno)) + geom_point(alpha=.1) + 
  facet_grid(profRank+DOM~DB+kk2Method) + scale_x_log10() + scale_y_log10()
```
bracken method is not good. Choose kraken default results
```{r}
SXR.sp.rep.abun.df <- dcast(SXR.r2m.bracken.more,
  DOM+DB+profRank+SAM+taxID+name+members+domain+phylum+findAnno+PBA_support+taxonSet~REP,
...
```{r}
SXR.SUS.sp.rep.df <- SXR.sp.rep.count.dfs
...
SXR.SUS.sp.rep.df$set <- factor(paste0(SXR.SUS.sp.rep.df$findAnno,SXR.SUS.sp.rep.df$PBA_support),
                                levels=c("foundTRUE","missTRUE", "foundFALSE"))
levels(SXR.SUS.sp.rep.df$set)<-c("PBAs found taxon. ","PBAs without taxon.","Taxon that PBA not covered")
SXR.SUS.sp.rep.stat <- ddply(SXR.SUS.sp.rep.df,c("profRank","SAM","DOM","DB","set"),summarise,cor=cor(RepX,RepY,method="s"))
SXR.SUS.sp.rep.stat$y <- ifelse(SXR.SUS.sp.rep.stat$SAM=="S",1e3,1e5)

ggplot(SXR.SUS.sp.rep.df,aes(x=RepX,y=RepY)) + geom_point(aes(color=SAM),alpha=.1) + 
  geom_text(data=SXR.SUS.sp.rep.stat,aes(x=1e1,y=y,color=SAM,label=paste0("r=",round(cor,2)))) +
  facet_grid(profRank+DOM~DB+set) + scale_x_log10() + scale_y_log10() + theme_bw()
```

# species with PBA support (pct.k)
```{r}
SXR.sp.rep.abun.use.df <- SXR.sp.rep.abun.dfs
SXR.sp.rep.abun.use.df$PBA_support[which(is.na(SXR.sp.rep.abun.use.df$PBA_support))] <- F
SXR.sp.rep.abun.use.df$set <- factor(paste0(SXR.sp.rep.abun.use.df$findAnno,SXR.sp.rep.abun.use.df$PBA_support),
                                levels=c("foundTRUE","missTRUE", "foundFALSE"))
levels(SXR.sp.rep.abun.use.df$set)<-c("PBAs found taxon.","PBAs without taxon. info","SILVA taxon that PBA not covered")

SXR.PBA.sp.rep.stat <- ddply(
  SXR.sp.rep.abun.use.df%>%filter(kk2Method=="pct.k"),c("DOM","SAM","profRank","DB","set"),summarise,
  cor=cor(RepX,RepY,method="s"),sp=length(unique(taxID)))

SXR.PBA.sp.rep.stat$y <- ifelse(SXR.PBA.sp.rep.stat$SAM=="S",1e-2,1e-1)

ggplot(SXR.sp.rep.abun.use.df%>%filter(kk2Method=="pct.k"),aes(x=RepX,y=RepY)) + geom_point(aes(color=SAM),alpha=.1) + 
  geom_text(data=SXR.PBA.sp.rep.stat,aes(x=1e-4,y=y,color=SAM,label=paste0(sp," nodes at ",profRank," level\nr=",round(cor,2)))) +
  facet_grid(DOM+profRank~DB+set) + scale_x_log10() + scale_y_log10() + theme_bw() + guides(color=F)
```
```{r}
SXR.PBA.sp.rep.stat <- ddply(
  SXR.sp.rep.abun.use.df%>%filter(kk2Method=="pct.k"&!grepl("uncultu",name)),c("DOM","SAM","profRank","DB","set"),summarise,
  cor=cor(RepX,RepY,method="s"),sp=length(unique(taxID)))

SXR.PBA.sp.rep.stat$y <- ifelse(SXR.PBA.sp.rep.stat$SAM=="S",1e-2,1e-1)

ggplot(SXR.sp.rep.abun.use.df%>%filter(kk2Method=="pct.k"&!grepl("uncultur",name)),aes(x=RepX,y=RepY)) +
  geom_point(aes(color=SAM),alpha=.1) + 
  geom_text(data=SXR.PBA.sp.rep.stat,aes(x=1e-4,y=y,color=SAM,label=paste0(sp," nodes at ",profRank," level\nr=",round(cor,2)))) +
  facet_grid(DOM+profRank~DB+set) + scale_x_log10() + scale_y_log10() + theme_bw() + guides(color=F)
...
SXR.sp.rep.abun.main.mosaic.df <- ddply(SXR.sp.rep.abun.main.mosaic.df,c("DOM","DB","profRank","SAM","taxonSet","RepX","RepY"),
...
if(do.write){
  ggsave(p.main.rep,width=7,height=5,file="../../Results/AUG20/FIG/p.main.rep.dot.pdf")
}
```
```{r}
...
SXR.rand.obs.5M.raw <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.rand.observeOTU.stat.tsv","fee3ef522ba1a8fc90c0b621faa84b18"),
  sep="\t",col.names = c("DOM","DB","SAM","size","count","annos","clades"))

SXR.rand.obs.500k.raw <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.000.rand.observeOTU.stat.tsv","c9621ec860d02e036929edaae8dbc60b"),
  sep="\t",col.names = c("DOM","DB","SAM","size","count","annos","clades"))
...
SXR.rand.PBA.obs.raw <- read.table(
  md5("../../Results/AUG20/STAT/KRAKEN/SXR.PBA.rand.observeOTU.stat.tsv","a1c7adec28534f9a9e8cda4cbf81d367"),
...
SXR.PBA.rand.obs.top <- ddply(SXR.PBA.rand.obs.melt%>%filter(SAM=="S"&group=="PBATAX"),c("DOM","DB","group"),summarise,top=max(count))
...
levels(SXR.PBA.rand.obs.melt$DB) <- c("PBAs","Combined","Public")
...
write.table(uniq.taxonSet.taxID.list[order(uniq.taxonSet.taxID.list$taxID),],
            "../../Results/AUG20/STAT/KRAKEN/uniq.taxonSet.taxID.list.tsv",
            sep="\t",quote=F,row.names = F)
    
ggsave(p.rarefraction.curve,width=10,height=3,
       file="../../Results/AUG20/FIG/p.rarefraction.curve.pdf")
