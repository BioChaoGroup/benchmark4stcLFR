...
MBR.ref.topRank.map <- rbind(
  cbind(CREF="NONE",MBR.BEAD.topRank.map),
  cbind(CREF="CLOSE",MBR.refbead.topRank.map)
)
...
```{r}
MBR.r2BEAD.b.sp.select <- MBR.r2BEAD.b.sp.anno%>%filter(spName!="")
MBR.r2BEAD.b.sp.select <- merge(MBR.r2BEAD.b.sp.select,
...
levels(MBR.r2BEAD.b.sp.select$spName) <- ifelse(levels(MBR.r2BEAD.b.sp.select$spName)%in%zymo.bac,levels(MBR.r2BEAD.b.sp.select$spName),"Others")
...
```
```{r}
MBR.r2BEAD.b.sp.ref.anno.stat.b <- MBR.r2BEAD.b.sp.ref.anno.stat%>%filter(LIB=="RCA"&CREF=="CLOSE"&tech=="BEAD")
