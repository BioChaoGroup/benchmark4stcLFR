...
```{bash,eval=FALSE}
mkdir -p ../../Results/AUG20/STAT/mash
mkdir -p ../../Results/AUG20/STAT/CLIP
mkdir -p ../../Results/AUG20/STAT/ANNO

# bac
metabbq smk -j -npk SAM/Z{1,2,3}/ANNO/clip2MockRef.bead.anno
for i in Z{1,2,3};do awk -v ID=$i '{print ID"\t"$0}' SAM/$i/ANNO/clip2MockRef.bead.anno;
done > ../../Results/AUG20/STAT/ANNO/MBR.clip2MockRef.bead.anno

for i in Z{1,2,3};do awk -v ID=$i '{print ID"\t"$0}' SAM/$i/ANNO/clip2MockRef.ref.cov;
done > ../../Results/AUG20/STAT/ANNO/MBR.clip2MockRef.ref.cov

for i in Z{1,2,3};do awk -v i=$i '{b=sprintf("BI%08d",FNR);print i"\t"b"\t"$0}' SAM/$i/mash/BI.msh.tsv;
done > ../../Results/AUG20/STAT/mash/MBR.BI.msh.tsv

# fungi
metabbq smk --configfile config.F.yaml -j -npk SAM/M{4,5,6}/ANNO/clip2MockRef.bead.anno
for i in M{4,5,6};do awk -v ID=$i '{print ID"\t"$0}' SAM/$i/ANNO/clip2MockRef.bead.anno;
done > ../../Results/AUG20/STAT/ANNO/MFR.clip2MockRef.bead.anno

for i in M{4,5,6};do awk -v ID=$i '{print ID"\t"$0}' SAM/$i/ANNO/clip2MockRef.ref.cov;
done > ../../Results/AUG20/STAT/ANNO/MFR.clip2MockRef.ref.cov

for i in M{4,5,6};do awk -v i=$i '{b=sprintf("BI%08d",FNR);print i"\t"b"\t"$0}' SAM/$i/mash/BI.msh.tsv;
done > ../../Results/AUG20/STAT/mash/MFR.BI.msh.tsv

#test
mkdir -p ../../Results/AUG20/STAT/READ
for i in Z{1,7};do awk -v ID=$i '{print ID"\t"$0}' SAM/$i/READ/fastp2MockRef.bam.depth;
done > ../../Results/AUG20/STAT/READ/fastp2MockRef.bam.depth
```
...
BM.BI.msh.tsv <- read.table(
  md5("../../Results/AUG20/STAT/mash/MBR.BI.msh.tsv","4d0d4fdf31a9229d495b16b38ffe42f3"),
  col.names = c("SID","BID","BB","rpb","kmers"))
FM.BI.msh.tsv <- read.table(
  md5("../../Results/AUG20/STAT/mash/MFR.BI.msh.tsv","3471149b554cf2de66d86906f3680174"),
  col.names = c("SID","BID","BB","rpb","kmers"))

# current res
##bac
zymo.cov <- read.table(
  md5("../../Results/AUG20/STAT/ANNO/MBR.clip2MockRef.ref.cov","21488d5e6a7e33e207a748d68cd48ede"),
  col.names = c("SID","species","pos","cov"))

BM.bead.anno <- read.table(
  md5("../../Results/AUG20/STAT/ANNO/MBR.clip2MockRef.bead.anno","dd8f9e48ad1e4b4fba0b1f3e7ddb288e"),sep="\t",
   col.names = c("SID","BID","BLen","asms","clips","tax.clips","tax.pieces","tax",
                 ...
                 "HYB","multiAnnos","bs.1st","bs.2nd","ident.1st","ident.2nd","tax.2nd"
))

#fungi
fungi.cov <- read.table(
  md5("../../Results/AUG20/STAT/ANNO/MFR.clip2MockRef.ref.cov","607c33739a8417192e06243bb20a98ae"),
  col.names = c("SID","species","pos","cov"))

FM.bead.anno <- read.table(
  md5("../../Results/AUG20/STAT/ANNO/MFR.clip2MockRef.bead.anno","5f59962bdd3a579077d1efd547139171"),sep="\t",
   col.names = c("SID","BID","BLen","asms","clips","tax.clips","tax.pieces","tax",
                 "ident","qMapLen","mismatch","gap","bitScore","bitScore2nd",
                 "sMapPieces","sMapLen","MAPDESC","strands","qMaps","sMaps",
                  "HYB","multiAnnos","bs.1st","bs.2nd","ident.1st","ident.2nd","tax.2nd"
...
#test
test.cov <- read.table("../../Results/AUG20/STAT/READ/fastp2MockRef.bam.depth",
                       col.names = c("SID","species","pos","cov"))
```
...
test.cov:
```{r}
shotgun.df <- data.frame(SID="t.shotgun",read.table("../../Results/AUG20/TEST/shotgun/read2MockRef.bam.depth",
...
zymo.fix.bed <- read.table("../../Source/REF/zymo/D6305.fix.rRNA.barrnap.bed",col.names = c(
 "species","from","to","unit","primary","strand","from2","to2","color"))
zymo.fix.cov <- read.table("../../Results/AUG20/STAT/CLIP/clip2MockRef.fix.ref.cov",
                           col.names = c("SID","species","pos","cov"))
...
```{r}
dist.Z1 <- read.table("../../Results/AUG20/SAM/Z1/CLIP/all.clip.rename.clust.debug.dist.tsv",
