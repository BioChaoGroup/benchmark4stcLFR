...
SFR.barrnap.gff3.tsv <- read.table(
  ...
  col.names = c("SID","BID","len18S","pct18S","len5.8S","pct5.8S","len28S","pct28S","unit")
)
...
SFR.silva.bead.anno <- read.table(
  md5("../../Results/AUG20/STAT/ANNO/SFR.CLIP.map.merge.bead.anno","3ed4e2810b59a9fdddd3c815e05ba5ef"),comment.char = "",sep="\t", quote = "", 
...
SFR.slv.bead.all.df$identG <- ifelse(
  ...
  ifelse(SFR.slv.bead.all.df$ident == 100,100,
         ifelse(SFR.slv.bead.all.df$ident >=99.5,99.5,
...
levels(pct.identG$identG) <- c("Missing",96,97,98,99,99.5,100)
...
```{r}
combine.unit.df <- merge(merge(SFR.itsx.df,SFR.F.subunit.df,by=c("SID","BID"),all.x=T),SFR.slv.bead.df[,c(1,2,28:30)],by=c("SID","BID"),all.x=T)
