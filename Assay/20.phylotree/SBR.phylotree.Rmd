...
for i in {S,O}{1,2,3};do
  perl -e '($tag,$f1,$f2)=@ARGV;open I,"< $f1";while(<I>){if($_=~/16S/&&$_!~/only \d+ percent of/){@s=split /\t/;$HS{$s[0]}++}};close I;
...
[tree2fa] start
  ...
  Written 14512 clade seqs. 9151 records cannot found seqs.
  SSU: 14509 (1105+13392+12) | LSU: 23(1+15+7) | BOTH:20(1+19+19)
...
```{r}
p2 <- ggtree(p.tree,layout="circular") + geom_tiplab(aes(label=anno2))
p2f <- p2%>%collapse(node=15143)%>%collapse(node=947611)
p4 <- ggtree(p.tree) + geom_tiplab(align=TRUE,aes(label=path),linesize=.1,hjust=0,size=1)

ggsave(p2,width=200,height=200,limitsize = FALSE,
       filename = "../../Results/DEC20/SAM/SBR/PREDICT/genus.onlyAnno.plot.pdf")
