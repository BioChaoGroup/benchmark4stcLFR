...
```{r}
mergedat <- left_join(tree%>%as_tibble(), anno1, by = 'label')
nonEuk.fug <- which(mergedat$domain!="domain__Eukaryota"|mergedat$kingdom!="kingdom__Fungi"|is.na(mergedat$kingdom)|is.na(mergedat$phylum)|is.na(mergedat$genus))
nhx_reduced <- drop.tip(mergedat%>%as.phylo(), nonEuk.fug) # 这里保留真菌数据
...
nhx_reduced$genus <- gsub("genus__", "", nhx_reduced$genus)
nhx_reduced$phylum <- gsub("phylum__", "", nhx_reduced$phylum)
nhx_reduced$phylum[grep("CLADE",nhx_reduced$phylum, ignore.case=T)] <- "Unknown__CLADE"

col <- c("#fcb9b2", "#998ec3","#91cf60","#b35806","#ff715b","#d8b365","#06aed5","#1a9850","blue","yellow")

ggtree(nhx_reduced%>%as.phylo(),color="gray",layout = "circular",size=0.1) %<+% nhx_reduced + geom_tiplab(aes(label=genus),size=0.5)
...
pn1 <- ggtree(y%>%as.phylo(), size=0.1,layout="circular") %<+% y + geom_tiplab(aes(label=genus),size=0.5)+geom_nodelab(aes(label=genus),geom = "text",size=0.5)#+geom_text2(aes(subset=!isTip, label=node), hjust=-0.3, size=0.5, color="black") + geom_tippoint(aes(color=phylum),alpha=1/4,size=4)
...
pn1 <- flip(pn1, 1144, 1250)
pn1 <- flip(pn1, 1251, 1323)
...
```{r}
mergedat <- left_join(tree%>%as_tibble(), anno1, by = 'label')
nonEuk <- which(mergedat$domain!="domain__Eukaryota"|is.na(mergedat$kingdom)|is.na(mergedat$phylum)|is.na(mergedat$genus)) # 有许多分支注释信息为NA，这里可以考虑将NA去掉
...
nhx_reduced$phylum <- gsub("phylum__", "", nhx_reduced$phylum)
nhx_reduced$phylum[grep("CLADE",nhx_reduced$phylum, ignore.case=T)] <- "Unknown__CLADE"
nhx_reduced$kingdom <- gsub("kingdom__", "", nhx_reduced$kingdom)
nhx_reduced$kingdom[grep("CLADE",nhx_reduced$kingdom, ignore.case=T)] <- "Unknown__CLADE"

# 仅显示真菌门类信息，其他界的门类信息用nonFungi表示
nhx_reduced[which(nhx_reduced$kingdom!="Fungi"),c(10)] <- "nonFungi"
...
y$genus <- as.character(y$genus)
pn1 <- ggtree(y%>%as.phylo(), size=0.1,layout="circular") %<+% y + geom_tiplab(aes(label=genus),size=0.5)+geom_nodelab(aes(label=genus),geom = "text",size=0.5)# 添加节点信息和点颜色 +geom_text2(aes(subset=!isTip, label=node), hjust=-0.3, size=0.5, color="black") + geom_tippoint(aes(color=phylum),alpha=1/4,size=4)
...
if(do.write){
  ggsave(x1, height = 20, width = 20, filename = "../../../Results/phylotree/Fungi/2.plot/Phylotree_Raxml_Eukaryota_Fungi_genus.pdf")
}
