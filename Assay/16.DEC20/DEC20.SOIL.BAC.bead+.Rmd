  ...
  for SAM in {S,O};do 
    for i in {000..111};do 
      if [ -f SAM/$DOM/KRAKEN/rand/$db.$SAM.$i.k2.bead.bracken ];then
        perl tmp.obs.pl "$DOM,$db,$SAM,$i" SAM/$DOM/KRAKEN/$db/data/topRank.map.gz SAM/$DOM/KRAKEN/rand/$db.$SAM.$i.k2.bead.bracken
      fi
  ...
  for SAM in {S,O};do 
    for i in {0..9};do 
      if [ -f SAM/$DOM/KRAKEN/rand/$db.$SAM.000.$i.k2.bead.bracken ];then
        perl tmp.obs.pl "$DOM,$db,$SAM,$i" SAM/$DOM/KRAKEN/$db/data/topRank.map.gz SAM/$DOM/KRAKEN/rand/$db.$SAM.000.$i.k2.bead.bracken
      fi
...
if(do.write){
  ...
  write.table(SXR.r2m.bracken.taxID,sep="\t",quote=F,row.names = F, eol = "\n",
              "../../Results/DEC20/STAT/KRAKEN/SXR.r2m.bracken.metadata.tsv")
}
...
SXR.rand.obs.5M.raw <- read.table(
  md5("../../Results/DEC20/STAT/KRAKEN/SXR.rand.observeOTU.stat.tsv","960ab508064c5407d8c5183d09d7d306"),
  sep="\t",col.names = c("DOM","DB","SAM","size","total_count","SHARE","PUBs","PBAs","NONE"))

SXR.rand.obs.500k.raw <- read.table(
  md5("../../Results/DEC20/STAT/KRAKEN/SXR.000.rand.observeOTU.stat.tsv","beb2df7c0d0b7e5d7461ae29d65730ad"),
  sep="\t",col.names = c("DOM","DB","SAM","size","total_count","SHARE","PUBs","PBAs","NONE"))

SXR.rand.obs.500k.raw$size <- (SXR.rand.obs.500k.raw$size+1)*0.5
#add a line of size 0
tmp.0.df <- ddply(SXR.rand.obs.500k.raw,c("DOM","DB","SAM"),summarise,size=0,total_count=0,
                  SHARE=0,PUBs=0,PBAs=0,NONE=NA)
SXR.rand.obs.500k.raw <- rbind(tmp.0.df,SXR.rand.obs.500k.raw)
...
SXR.rand.obs.melt <- melt(SXR.rand.obs.raw,
                          measure.vars = c("SHARE","PBAs","PUBs"),
...
SXR.rand.obs.melt <- ddply(SXR.rand.obs.melt,c("DOM","DB","SAM","group"),sFUN)

```

```{r}

SXR.rand.obs.top <- ddply(SXR.rand.obs.melt%>%filter(SAM=="S"),c("DOM","DB","group"),summarise,top=max(count))
p.rarefraction2.curve1 <- ggplot(SXR.rand.obs.melt%>%filter(SAM=="S"),aes(x=size,y=count,color=group)) +
  geom_line(aes(linetype=DOM),size=1) + scale_color_brewer(palette="Set1") +
  geom_text(data=SXR.rand.obs.melt%>%filter(top95==T&SAM=="S"),
            aes(label=paste0("(",size,",",count,")"))) +
  geom_point(data=SXR.rand.obs.melt%>%filter(top95==T&SAM=="S"),shape=4,size=6) +
  scale_y_continuous(breaks=c(seq(400,1400,200),2000,seq(0,100000,10000))) +
  scale_x_continuous(breaks=seq(0,600,50)) + theme_bw() 

p.rarefraction2.curve1
```
```{r}
p.rarefraction2.curve2 <- ggplot(SXR.rand.obs.melt%>%filter(SAM=="S"),aes(x=size,y=count,color=group)) +
  geom_line(aes(linetype=DOM),size=1) + scale_color_brewer(palette="Set1") +
  geom_text(data=SXR.rand.obs.melt%>%filter(top95==T&SAM=="S"),
            aes(label=paste0("(",size,",",count,")"))) +
  geom_point(data=SXR.rand.obs.melt%>%filter(top95==T&SAM=="S"),shape=4,size=6) +
  scale_y_continuous(breaks=c(seq(0,2000,200),2000,seq(0,100000,10000)),limits=c(0,2000)) +
  scale_x_continuous(breaks=seq(0,600,50)) + theme_bw()
...
#p.rarefraction2.curve1 <- ggplot(SXR.PBA.rand.obs.melt%>%filter(SAM=="S"&group%in%pick.grp&DB=="BEAD+"), aes(x=size,y=count,color=group)) +
  #geom_hline(data=SXR.PBA.rand.obs.top,aes(yintercept = top*.95,color=DB,linetype=DOM),alpha=.5,size=1) +
...
#p.rarefraction2.curve2 <- ggplot(SXR.PBA.rand.obs.melt%>%filter(SAM=="S"&group%in%pick.grp&DB=="Combined"), aes(x=size,y=count,color=group)) +
  #geom_hline(data=SXR.PBA.rand.obs.top,aes(yintercept = top*.95,color=DB,linetype=DOM),alpha=.5,size=1) +
